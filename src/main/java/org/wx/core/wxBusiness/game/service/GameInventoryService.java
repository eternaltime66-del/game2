...
public class GameInventoryService extends WxServiceImpl<GameInventoryMapper, GameInventory> {
    ...
    /**
     * 后台调整物品堆叠上限后，重排所有持有该物品玩家的仓库格子。
     */
    @Transactional(rollbackFor = Exception.class)
    public void restackItemInAllWarehouses(String itemId) {
        ...
        ErrorFactory.throwError(maxStack <= 0, "堆叠上限无效");

        List<String> uids = this.find()
                .eq(GameInventory::getItemId, itemId)
                .list()
                .stream()
                .map(GameInventory::getUid)
                .filter(Objects::nonNull)
                .distinct()
                .toList();
        for (String uid : uids) {
            restackItemInWarehouse(uid, itemId, item, maxStack);
        }
    }

    private void restackItemInWarehouse(String uid, String itemId, GameItem item, int maxStack) {
        List<GameInventory> itemRows = this.find()
                .eq(GameInventory::getUid, uid)
                .eq(GameInventory::getItemId, itemId)
        ...
        int totalQty = itemRows.stream()
                .mapToInt(row -> row.getQuantity() != null ? row.getQuantity() : 0)
                .sum();
        if (totalQty <= 0) {
            itemRows.forEach(row -> this.removeById(row.getId()));
            return;
        }

        int stacksNeeded = (totalQty + maxStack - 1) / maxStack;
        List<GameInventory> allRows = listByUid(uid);
        Set<Integer> usedSlots = allRows.stream()
                .filter(row -> !itemId.equals(row.getItemId()))
                .map(GameInventory::getSlotNo)
                .collect(Collectors.toSet());

        GameWarehouse warehouse = warehouseService.getOrInit(uid);
        int maxSlots = warehouse.getMaxSlots() != null ? warehouse.getMaxSlots() : GameWarehouse.DEFAULT_MAX_SLOTS;
        int freeSlots = maxSlots - usedSlots.size();
        ErrorFactory.throwError(stacksNeeded > freeSlots,
                "玩家仓库格子不足，无法按新堆叠上限重排「" + item.getName() + "」");

        if (!needsWarehouseRestack(itemRows, maxStack, stacksNeeded)) {
            return;
        }

        for (GameInventory row : itemRows) {
            this.removeById(row.getId());
        }

        int remaining = totalQty;
        while (remaining > 0) {
            Integer slotNo = findNextEmptySlot(usedSlots, maxSlots);
            ErrorFactory.notNull(slotNo, "玩家仓库格子不足，无法按新堆叠上限重排「" + item.getName() + "」");
    ...
    private boolean needsWarehouseRestack(List<GameInventory> itemRows, int maxStack, int stacksNeeded) {
        if (itemRows.size() != stacksNeeded) {
            return true;
        }
        for (int i = 0; i < itemRows.size(); i++) {
            int qty = itemRows.get(i).getQuantity() != null ? itemRows.get(i).getQuantity() : 0;
            boolean isLast = i == itemRows.size() - 1;
            if (qty > maxStack) {
                return true;
            }
            if (!isLast && qty < maxStack) {
                return true;
            }
        }
        return false;
